...
public:
    ...
    void PathPlanning(Point_5D _startPoint, Point_5D _targetPoint, vector<Point_5D>& path);
    bool CheckCollision(Mat _LabelMap, int x, int y, double theta, double length);
    void RobotKinematics(Mat neighbor,Mat _LabelMap,int &x,int &y, double &theta_, double &length_,double &alpha_,int cx,int cy,double ct,double cl,int k);
    void DrawPath(Mat& _Map, vector<Point_5D>& path, InputArray Mask = noArray(), Scalar color = Scalar(0, 0, 255),
            int thickness = 1, Scalar maskcolor = Scalar(255, 255, 255));
    void Smooth(vector<Point_5D>& path);
private:
    void MapProcess(Mat& Mask);
    Node* FindPath();
    void GetPath(Node* TailNode, vector<Point_5D>& path);
